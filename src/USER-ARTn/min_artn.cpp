...
MinARTn::MinARTn(LAMMPS *lmp) : MinLineSearch(lmp)
{
  ...
  if (pressure_compute < 0)
    error->all(FLERR, "Could not find compute pressure ID");
...
int MinARTn::iterate(int maxevent)
{
  ...
  if (!min_fire)
    stop_condition = min_converge(max_conv_steps, 0);
  else
  ...
  if (events_per_atom != 0)
    max_num_events = events_per_atom * ngroup;
  ...
  while (1)
  {
    ...
    if (flag_relax_sad)
      sad_converge(max_conv_steps);

    ++ievent;
    ++stage;
    if (me == 0 && fp2)
      fprintf(fp2, "%5d %9.6f %7.3f", ievent, delE, egval);
...
int MinARTn::check_sad2min()
{
  ...
  if (ddum >= disp_sad2min_thr && delE > 0.)
  {
    ...
    if (tmp_all[1] > 0.)
      ...
      for (int i = 0; i < nvec; ++i)
        fperp[i] = -egvec[i] * push_over_saddle;
  }
  else
  {
    if (me == 0)
...
int MinARTn::push_back_sad()
{
  ++stage;
  for (int i = 0; i < nvec; ++i)
    x0tmp[i] = xvec[i]; // x0tmp stores the saddle position
...
void MinARTn::analysis_saddle()
{
  ...
  if (me == 0 && fp2)
    fprintf(fp2, " %5d %7.2f %7.2f %7.2f %7.2f", n_movedall, sqrt(tempall[0]), sqrt(tempall[1]), sqrt(tempall[2]), sqrt(tempall[0] + tempall[1] + tempall[2]));
  if (flag_sadl_press)
  {
    double *vvec = atom->v[0];
    for (int i = 0; i < nvec; ++i)
...
void MinARTn::push_down()
{
  ...
  if (!min_fire)
  {
    SD_min_converge(SD_steps, 1);
    evalf += neval;
    stop_condition = min_converge(max_conv_steps, 1);
    evalf += neval;
  }
  else
  {
...
void MinARTn::metropolis()
{
  ...
  if (me == 0 && fp2)
    fprintf(fp2, " %5d %4d %5d %7d %10.3f %10.3f", ref_id, sad_id, min_id, that, eref, ecurrent);
...
void MinARTn::read_control()
{
  ...
  char *fmin, *fsad, *fproc;
  fmin = fsad = fproc = NULL;
  if (fp == NULL)
  {
  ...
  else
  {
    while (1)
    {
      ...
      if (strcmp(token1, "random_seed") == 0)
      {
        ...
        if (seed < 1)
          error->all(FLERR, "ARTn: seed must be greater than 0");
      }
      ...
      else if (strcmp(token1, "max_num_events") == 0)
      {
        ...
        if (max_num_events < 1)
          error->all(FLERR, "ARTn: max_num_events must be greater than 0");
      }
      else if (strcmp(token1, "max_activat_iter") == 0)
      {
        max_activat_iter = utils::inumeric(FLERR, token2, false, lmp);
        if (max_activat_iter < 1)
          error->all(FLERR, "ARTn: max_activat_iter must be greater than 0");
      }
      ...
      else if (strcmp(token1, "increment_size") == 0)
      {
        ...
        if (increment_size <= 0.)
          error->all(FLERR, "ARTn: increment_size must be greater than 0.");
      }
      ...
      else if (!strcmp(token1, "atom_disp_thr"))
      {
        ...
        if (atom_disp_thr <= 0.)
          error->all(FLERR, "ARTn: atom_disp_thr must be greater than 0.");
      }
      else if (strcmp(token1, "max_perp_moves_c") == 0)
      {
        max_perp_moves_c = utils::inumeric(FLERR, token2, false, lmp);
        if (max_perp_moves_c < 1)
          error->all(FLERR, "ARTn: max_perp_moves_c must be greater than 0.");
      }
      else if (strcmp(token1, "force_th_perp_sad") == 0)
      {
        force_th_perp_sad = utils::numeric(FLERR, token2, false, lmp);
        if (force_th_perp_sad <= 0.)
          error->all(FLERR, "ARTn: force_th_perp_sad must be greater than 0.");
      }
      ...
      else if (!strcmp(token1, "max_ener_tol"))
      {
        ...
        if (max_ener_tol <= 0.)
          error->all(FLERR, "ARTn: max_ener_tol must be greater than 0.");
      }
      ...
      else if (strcmp(token1, "flag_dump_direction") == 0)
      {
        ...
        // flag_delta_direction = utils::inumeric(FLERR, token2,false,lmp);
      ...
      else if (strcmp(token1, "deformation_gradient") == 0)
      {
        ...
        {
          flag_random_deformation_gradient = 1;
        }
        else
        {
          for (int i = 0; i < 9; ++i)
  ...
  if (ngroup < 1)
    error->all(FLERR, "No atom is found in your desired group for activation!");
  ...
  {
    if (strcmp("ARTnmin", output->dump[idump]->id) == 0)
      break;
  }
  ...
  {
    // dump_min already defined in the lammps and use that one
    if (dump_min_every)
    {
  ...
  for (idump = 0; idump < output->ndump; idump++)
  {
    if (strcmp("ARTnsad", output->dump[idump]->id) == 0)
      break;
  }
  if (idump == output->ndump)
  {
  ...
  {
    // dump_sad already defined in the lammps and use that one
    if (dump_sad_every)
    {
  ...
  if (strcmp(fproc, "NULL") != 0)
  {
    ...
    strcpy(tmp[4], fproc);
    if (dump_event_every)
      dumpevent = new DumpAtom(lmp, 5, tmp);
  }
...
void MinARTn::artn_init()
{
  ...
  int *mask = atom->mask;
  int *llist;
  memory->create(llist, MAX(1, atom->nlocal), "llist");
  ...
  if (nall != ngroup)
    error->all(FLERR, "# of atoms in group mismatch!");
  if (me == 0)
    memory->create(glist, ngroup, "glist");
  ...
  if (dumpmin && !dumpmin_outside)
    dumpmin->init();
  if (dumpsad && !dumpsad_outside)
    dumpsad->init();
  if (dumpevent)
    dumpevent->init();
...
int MinARTn::find_saddle()
{
  ...
  for (int it = 0; it < max_iter_basin; ++it)
  {
    ...
    while (1)
    {
      ...
      if (fperp2all < force_thh_p2 || m_perp > nmax_perp || nfail > 5)
    ...
    if (me == 0)
    {
      ...
      ftot = sqrt(ftotall);
      delr = sqrt(delr);
      if (fp1 && log_level && it % print_freq == 0)
        fprintf(fp1, "%8d %10.5f %3d %3d %5d %10.5f %10.5f %10.5f %10.5f %10.5f " BIGINT_FORMAT "\n", it,
                delE, m_perp, trial, nlanc, ftot, fpar2all, fperp2, egval, delr, evalf);
      if (screen && it % print_freq == 0)
        fprintf(screen, "%8d %10.5f %3d %3d %5d %10.5f %10.5f %10.5f %10.5f %10.5f " BIGINT_FORMAT "\n", it,
                delE, m_perp, trial, nlanc, ftot, fpar2all, fperp2, egval, delr, evalf);
    }
    ...
    if (it > min_num_ksteps && egval < eigen_th_well)
    {
      ...
      if (me == 0)
      {
        if (fp1 && log_level && it % print_freq)
          fprintf(fp1, "%8d %10.5f %3d %3d %5d %10.5f %10.5f %10.5f %10.5f %10.5f " BIGINT_FORMAT "\n", it,
                  delE, m_perp, trial, nlanc, ftot, fpar2all, fperp2, egval, delr, evalf);
        if (screen && it % print_freq)
          fprintf(screen, "%8d %10.5f %3d %3d %5d %10.5f %10.5f %10.5f %10.5f %10.5f " BIGINT_FORMAT "\n", it,
                  delE, m_perp, trial, nlanc, ftot, fpar2all, fperp2, egval, delr, evalf);
    ...
    for (int i = 0; i < nvec; ++i)
      xvec[i] += MIN(dmax, MAX(-dmax, step * h[i]));
  }
  ...
  if (flag == 0)
  {
    if (me == 0)
    {
      if (fp1 && log_level && (max_iter_basin - 1) % print_freq)
        fprintf(fp1, "%8d %10.5f %3d %3d %5d %10.5f %10.5f %10.5f %10.5f %10.5f " BIGINT_FORMAT "\n", (max_iter_basin - 1),
                delE, m_perp, trial, nlanc, ftot, fpar2all, fperp2, egval, delr, evalf);
      if (screen && (max_iter_basin - 1) % print_freq)
        fprintf(screen, "%8d %10.5f %3d %3d %5d %10.5f %10.5f %10.5f %10.5f %10.5f " BIGINT_FORMAT "\n", (max_iter_basin - 1),
                delE, m_perp, trial, nlanc, ftot, fpar2all, fperp2, egval, delr, evalf);
  ...
  for (int it_s = 0; it_s < max_activat_iter; ++it_s)
  {
    ...
    else
    {
      ...
      while (1)
      {
        ...
        // condition to break
        if (fperp2all < force_thc_p2 || m_perp > max_perp || nfail > 5)
          break;
    ...
    {
      fperp2 = sqrt(fperp2all);
      if (fp1 && log_level && it_s % print_freq == 0)
        fprintf(fp1, "%8d %10.5f %3d %3d %5d %10.5f %10.5f %10.5f %8.4f %8.4f %6.3f " BIGINT_FORMAT "\n",
                it_s, delE, m_perp, trial, nlanc, ftotall, fpar2all, fperp2, egval, delr, hdotall, evalf);
      if (screen && it_s % print_freq == 0)
        fprintf(screen, "%8d %10.5f %3d %3d %5d %10.5f %10.5f %10.5f %8.4f %8.4f %6.3f " BIGINT_FORMAT "\n",
                it_s, delE, m_perp, trial, nlanc, ftotall, fpar2all, fperp2, egval, delr, hdotall, evalf);
    }
    ...
    if (egval > eigen_th_fail || delE < -0.1)
    {
      if (me == 0)
      {
        if (fp1 && log_level && it_s % print_freq)
          fprintf(fp1, "%8d %10.5f %3d %3d %5d %10.5f %10.5f %10.5f %8.4f %8.4f %6.3f " BIGINT_FORMAT "\n",
                  it_s, delE, m_perp, trial, nlanc, ftotall, fpar2all, fperp2, egval, delr, hdotall, evalf);
        if (screen && it_s % print_freq)
          fprintf(screen, "%8d %10.5f %3d %3d %5d %10.5f %10.5f %10.5f %8.4f %8.4f %6.3f " BIGINT_FORMAT "\n",
                  it_s, delE, m_perp, trial, nlanc, ftotall, fpar2all, fperp2, egval, delr, hdotall, evalf);
...
#define MinEGV 0.5 // was 0.5
    ...
    for (int i = 0; i < nvec; ++i)
      xvec[i] += MIN(dmax, MAX(-dmax, factor * egvec[i]));
    ecurrent = energy_force(1);
    ++evalf;
    artn_reset_vec();
    reset_x00();
...
void MinARTn::random_kick()
{
  ...
  if (fabs(cluster_radius) < ZERO)
  { // only the cord atom will be kicked
    for (int i = 0; i < nlocal; ++i)
    {
      if (tag[i] == that)
      {
  ...
  else if (cluster_radius < 0.)
  { // all atoms in group will be kicked
    for (int i = 0; i < nlocal; ++i)
    {
  ...
  { // only atoms within a radius to the central atom will be kicked
    double one[3];
    one[0] = one[1] = one[2] = 0.;
    for (int i = 0; i < nlocal; ++i)
    {
    ...
    if (flag_deformation_gradient && flag_random_deformation_gradient)
    {
      // creat a random orthogonal tensor
      if (me == 0)
      {
        double *tmp, sum, dotsum, dotsum1, dotsum2;
        int i;
        tmp = new double[9];
        // a random tensor
        for (i = 0; i < 9; ++i)
        {
          tmp[i] = random->uniform();
        }
        // normolize the first vector
        sum = 0.;
        for (i = 0; i < 3; ++i)
        {
          sum = sum + tmp[i] * tmp[i];
        ...
        for (i = 3; i < 6; ++i)
        {
          dotsum = dotsum + tmp[i] * tmp[i - 3];
        }
        for (i = 3; i < 6; ++i)
        {
          tmp[i] = tmp[i] - dotsum * tmp[i - 3];
        }
        sum = 0.;
        for (i = 3; i < 6; ++i)
        {
        ...
        for (i = 6; i < 9; ++i)
        {
          dotsum1 = dotsum1 + tmp[i] * tmp[i - 3];
          dotsum2 = dotsum2 + tmp[i] * tmp[i - 6];
        }
        for (i = 6; i < 9; ++i)
        {
          tmp[i] = tmp[i] - dotsum1 * tmp[i - 3] - dotsum2 * tmp[i - 6];
        }
        sum = 0.;
        for (i = 6; i < 9; ++i)
        {
        ...
        for (i = 0; i < 9; ++i)
        {
          deformation_gradient[i] = tmp[i];
        }
        delete[] tmp;
      }
      // broadcast the tensor
      MPI_Bcast(deformation_gradient, 9, MPI_DOUBLE, 0, world);
    }
  ...
  {
    if (me == 0)
      error->one(FLERR, "Zero norm in random_kick");
  }
  ...
  if (me == 0 && idum < 1)
    error->one(FLERR, "No atom to kick!");
  if (me == 0)
    print_info(19);
...
int MinARTn::new_min_perp_fire(int maxiter)
{
  ...
  for (int iter = 0; iter < maxiter; ++iter)
  {
    ...
    for (int i = 0; i < nvec; ++i)
      fperp[i] = fvec[i] - (1 + para_factor) * fdothall * h[i];
...
void MinARTn::print_info(const int flag)
{
  ...
  {
    if (fp1)
      fprintf(fp1, " with total %d atoms. %d success till now.\n", idum, sad_id);
    if (screen)
      fprintf(screen, " with total %d atoms. %d success till now.\n", idum, sad_id);
  }
  else if (flag == 20)
  {
    if (fp1)
      fprintf(fp1, "    The distance between new found saddle and min-%d is %g, > %g, acceptable.\n", ref_id, ddum, disp_sad2min_thr);
    if (screen)
      fprintf(screen, "    The distance between new found saddle and min-%d is %g, > %g, acceptable.\n", ref_id, ddum, disp_sad2min_thr);
  }
  else if (flag == 21)
  {
    if (fp1)
      fprintf(fp1, "    The distance between new saddle and min-%d is %g, < %g, rejected.\n", ref_id, ddum, disp_sad2min_thr);
    if (screen)
      fprintf(screen, "    The distance between new saddle and min-%d is %g, < %g, rejected.\n", ref_id, ddum, disp_sad2min_thr);
  }
  else if (flag == 30)
  {
    if (fp1)
      fprintf(fp1, "  Stage %d, push back the saddle to confirm if it is linked with min-%d\n", stage, ref_id);
    if (screen)
      fprintf(screen, "  Stage %d, push back the saddle to confirm if it is linked with min-%d\n", stage, ref_id);
  }
  else if (flag == 31)
  {
    if (fp1)
  ...
  {
    if (fp1)
      fprintf(fp1, "  Stage %d failed, |Ecurrent - Eref| = %g > %g, reject the new saddle.\n", stage, ddum, max_ener_tol);
    if (screen)
      fprintf(screen, "  Stage %d failed, |Ecurrent - Eref| = %g > %g, reject the new saddle.\n", stage, ddum, max_ener_tol);
  }
  else if (flag == 33)
  {
    if (fp1)
      fprintf(fp1, "  Stage %d succeeded, dr = %g, drmax_atom = %g, tolerance = %g, accept the new saddle.\n", stage, ddum, tmpddum, max_disp_tol);
    if (screen)
      fprintf(screen, "  Stage %d succeeded, dr = %g, drmax_atom = %g, tolerance = %g, accept the new saddle.\n", stage, ddum, tmpddum, max_disp_tol);
  }
  else if (flag == 34)
  {
    if (fp1)
      fprintf(fp1, "  Stage %d failed, dr = %g, drmax_atom = %g, tolerance = %g, reject the new saddle.\n", stage, ddum, tmpddum, max_disp_tol);
    if (screen)
      fprintf(screen, "  Stage %d failed, dr = %g, drmax_atom = %g, tolerance = %g, reject the new saddle.\n", stage, ddum, tmpddum, max_disp_tol);
  }
  else if (flag == 40)
  {
    if (fp1)
      fprintf(fp1, "  Stage %d, further relax the newly found sad-%d ...\n", stage, sad_id);
    if (screen)
      fprintf(screen, "  Stage %d, further relax the newly found sad-%d ...\n", stage, sad_id);
  }
  else if (flag == 41)
  {
    if (fp1)
  ...
  {
    if (fp1)
      fprintf(fp1, "  Stage %d, push over the new saddle, Enew: %g; Eref= %g. Relaxing...\n", stage, ecurrent, eref);
    if (screen)
      fprintf(screen, "  Stage %d, push over the new saddle, Enew: %g; Eref= %g. Relaxing...\n", stage, ecurrent, eref);
  }
  else if (flag == 51)
  {
    if (fp1)
  ...
  {
    if (fp1 && log_level)
      fprintf(fp1, "      - Distance to min-%8.8d  : %g\n", ref_id, ddum);
    if (screen && log_level)
      fprintf(screen, "      - Distance to min-%8.8d  : %g\n", ref_id, ddum);
  }
  else if (flag == 61)
  {
    if (fp1)
      fprintf(fp1, "  Stage %d done, the new min (E= %g) of ID  %d is accepted by Metropolis.\n", stage, ecurrent, min_id);
    if (screen)
      fprintf(screen, "  Stage %d done, the new min (E= %g) of ID %d is accepted by Metropolis.\n", stage, ecurrent, min_id);
  }
  else if (flag == 62)
  {
    if (fp1)
      fprintf(fp1, "  Stage %d done, the new min (E= %g) of ID %d is rejected by Metropolis.\n", stage, ecurrent, min_id);
    if (screen)
      fprintf(screen, "  Stage %d done, the new min (E= %g) of ID %d is rejected by Metropolis.\n", stage, ecurrent, min_id);
  }
  return;
}
  ...
  for (int iter = 0; iter < maxiter; ++iter)
  {
    ...
    if (vdotfall > 0.)
    {
      ...
      if (fdotfall == 0.)
        ...
        scale2 = alpha * sqrt(vdotvall / fdotfall);
      for (int i = 0; i < nvec; ++i)
        vvec[i] = scale1 * vvec[i] + scale2 * fvec[i];
...
void MinARTn::read_delta_direction(char *file, double *delpos)
{
  ...
  for (int i = 0; i < dimension * natoms; ++i)
    filepos[i] = 0.0;
  if (me == 0)
  {
    fp = fopen(file, "r");
...
void MinARTn::read_dump_direction(char *file, double *delpos)
{
  ...
  if (me == 0)
  {
    ...
    fgets(oneline, MAXLINE, fp);
    if (oneline[strlen("ITEM: ATOMS id type x")] == 's')
      flag_scale = 1;
